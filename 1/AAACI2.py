...
# # # # # # # # # # # preparing the labeled and unlabeled train data # # # # # # # # # #
    ...
    for num_ctrs in [20]:
        ...
        for thresh1 in [  0.0001, 0.001, 0.01, 0.1]:
            ...
            # for kk in range(30):
                # plt.subplot(2, 3, 1)
                # plt.imshow(mask_deformable_[kk].squeeze(), cmap='gray')
                # plt.title('mask deformable')
                ...
                # plt.subplot(2, 3, 4)
                # plt.imshow(mask_PL[kk], cmap='gray')
                # plt.title('pred mask')
                
                # plt.subplot(2, 3, 5)
                # plt.imshow(final_deformable[kk], cmap='gray')
                # plt.title('final deformable')

                # plt.subplot(2, 3, 6)
                # plt.imshow(reliable_mask[kk], cmap='gray')
                # plt.title('reliable mask')
    ...
    ## # # # # # # # # # # # # # # # # # # # apply deformable models # # # # # # # # # # # # # # # # # # # # # # # 
    # for b in range(len(img_dev)):     
    
        # a = get_largest_connected_component (img_dev[b].squeeze(), pred_masks_dev[b].squeeze()) 
    ...
    # for shape_weight in shape_weights:  
        # for TV_weight in TV_weights:
            ...
            # for index in unlabeled_indices:
                ...
                # mask[mask<0.5] = 0
                # mask [ mask>=0.5] = 1
                # # mask = get_largest_connected_component(img, mask)
            ...
            # if(deftmp[0]>best_dice):
                # best_dice = deftmp[0]
                # best_masks_deformable = masks_deformable
    ...
    # # # # ## # # # # # # # # # # # # # # # discarding unreliable images # # # # # # # # # # # # # # 
    # print("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU")  
    # for index_number in range(len(indices_ui)):
        # # print(index_number)
        # indice_ui = indices_ui[index_number]
        # mask_all_ui = pred_masks_all[indice_ui]
        # img_all_ui = img_all [indice_ui]
        # save_folder = 'unreliable_images\\'+ str(num_ctrs)+'\\'+ str(index_number) +'\\weights\\'+str(itr)
        # UItmp,UIepoch_tmp = (train(img_all_ui, mask_all_ui, img_dev, mask_dev, save_folder))
        # print(UItmp)
        # print(len(img_all_ui))
        # print("*")
        # save_results ('UI\\'+ str(num_ctrs)+'\\'+str(index_number)+'\\results','indices'+str(itr)+'.txt', [UItmp]) 
        ...
        # # pred_masks_dev[pred_masks_dev>=0.5] = 1
        # # pred_masks_dev[pred_masks_dev<0.5] = 0
        # # save_images_with_contours(img_dev, pred_masks_dev, mask_dev, output_dir = 'unreliable_images' + str(thresh_numbers))            
        
   
        # # # time.sleep(300) # 300 seconds = 5 minutes
    
    # # # # # # # # # # # # # # # # # # # discarding unreliable pixels # # # # # # # # # # # # # # # # 
    
    # # # save_folder = 'PL2\\'+ str(num_ctrs)+'\\filter0.75\\weights\\'+str(itr)
    # # # PLtmp,PLepoch_tmp = (train(img_all, mask_all, img_dev, mask_dev, save_folder, pixel_filtering = True, thresh = 0.75))
    # # # save_results ('PL2\\'+ str(num_ctrs)+'\\filter0.75\\results','indices'+str(itr)+'.txt', [PLtmp])    
    # # # print (PLtmp)
    
    
    # # # save_folder = 'PL2\\'+ str(num_ctrs)+'\\filter0.99\\weights\\'+str(itr)
    # # # PLtmp,PLepoch_tmp = (train(img_all, mask_all, img_dev, mask_dev, save_folder, pixel_filtering = True, thresh = 0.99))
    # # # print(PLtmp)
    # # # save_results ('PL2\\'+ str(num_ctrs)+'\\filter0.99\\results','indices'+str(itr)+'.txt', [PLtmp])    
    # # # # time.sleep(300) # 300 seconds = 5 minutes   

    # # # save_folder = 'PL2\\'+ str(num_ctrs)+'\\filter0.9999\\weights\\'+str(itr)
    # # # PLtmp,PLepoch_tmp = (train(img_all, mask_all, img_dev, mask_dev, save_folder, pixel_filtering = True, thresh = 0.9999))
    # # # print(PLtmp)
