...
def custom_loss(yignore):
    def loss(ytrue, ypred):
        yignore = tf.cast(yignore, tf.float32)
        loss_ce = tf.nn.sigmoid_cross_entropy_with_logits(labels=ytrue, logits=ypred)
        loss_ce = loss_ce * (1 - yignore)
        loss_ce = tf.reduce_mean(loss_ce)
        
        ypred_dx = ypred[:, :-1, :] - ypred[:, 1:, :]
        ypred_dy = ypred[:, :, :-1] - ypred[:, :, 1:]
        loss_tv = tf.reduce_mean(tf.abs(ypred_dx)) + tf.reduce_mean(tf.abs(ypred_dy))
...
def weak_loss (epoch):                               
    def weak_annotation (y_true , y_pred ):
        ...
        if (epoch<10) : 
            ...
            sample_weight = epoch/20 * (1-SUP)*0 + SUP
        epsilon = K.epsilon()
